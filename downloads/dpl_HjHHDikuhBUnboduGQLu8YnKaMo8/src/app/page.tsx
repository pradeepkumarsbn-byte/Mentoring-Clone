...
function InvitationsView({ data, save, busy, setModal }: { data: State; save: (action: string, payload: Record<string, unknown>, success: string, key?: string) => Promise<boolean>; busy: string | null; setModal: (modal: Modal) => void }) {
  ...
  const [query, setQuery] = useState("");
  const [mentor, setMentor] = useState("all");
  const [callFilter, setCallFilter] = useState("remaining");
  ...
  const typePrograms = latestPrograms(allTypePrograms, allTypePrograms.length);
  const target = typePrograms.find((program) => program.id === targetProgramId) ?? typePrograms[0];
  const eligible = eligibleInvitees(data, typeId, mentor, query);
  const eligibleIds = new Set(eligible.map(boy => boy.id));
  const targetInvitations = target ? data.invitations.filter(item => item.programId === target.id && eligibleIds.has(item.boyId)) : [];
  const calledIds = new Set(targetInvitations.map(item => item.boyId));
  const remaining = eligible.filter(boy => !calledIds.has(boy.id));
  const visible = callFilter === "remaining" ? remaining : callFilter === "called" ? eligible.filter(boy => calledIds.has(boy.id)) : eligible;
  const expected = targetInvitations.filter((item) => parseInvitationResponse(item.response).category === "Coming").length;
  return <div className="view-panel"><div className="invitation-hero"><div><p className="eyebrow">{siteText(data, "invitations_eyebrow", "Invitation responses")}</p><h2>{siteText(data, "invitations_title", "Call, invite and record the answer")}</h2><p>{siteText(data, "invitations_description", "Every invitation requires a response. Add the boy's exact reason in the note so future follow-up is clear.")}</p></div><div className="invitation-outcomes"><div><strong>{eligible.length}</strong><span>eligible / {remaining.length} remaining</span></div><div><strong>{targetInvitations.length}</strong><span>called</span></div><div className="coming"><strong>{expected}</strong><span>coming</span></div></div></div>
    <div className="invite-controls"><div className="program-switcher">{data.programTypes.map((type) => <button key={type.id} className={typeId === type.id ? "active" : ""} onClick={() => { setTypeId(type.id); setTargetProgramId(""); }}>{type.name}</button>)}</div><div className="invite-filters"><Search value={query} onChange={setQuery} placeholder="Search eligible boys…" /><select aria-label="Mentor" value={mentor} onChange={(event) => setMentor(event.target.value)}><option value="all">All mentors</option>{data.mentors.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select aria-label="Program session" value={target?.id ?? ""} onChange={(event) => setTargetProgramId(event.target.value)}><option value="">Select dated session</option>{typePrograms.map((program) => <option key={program.id} value={program.id}>{formatDate(program.date)} · {program.venue}</option>)}</select><select aria-label="Invitation call status" value={callFilter} onChange={event => setCallFilter(event.target.value)}><option value="remaining">Remaining to call ({remaining.length})</option><option value="called">Already called ({targetInvitations.length})</option><option value="all">All eligible ({eligible.length})</option></select></div></div>
    <div className="eligibility-rule"><span>✓</span><p><b>{selectedType?.name ?? "Program"} rule:</b> repeat dates are combined. Present or Late once means no longer eligible for this Program Type. Dropped boys never appear in future invitation lists.</p></div>
    {!target ? <div className="panel inline-empty"><Empty text={`Add a dated ${selectedType?.name ?? "program"} session before recording invitations.`} /><button className="primary-button" onClick={() => setModal("program")}>＋ Add program</button></div> : <div className="invite-grid">{visible.map((boy) => { const invitation = data.invitations.find((item) => item.programId === target.id && item.boyId === boy.id); return <InvitationResponseCard key={`${target.id}:${boy.id}:${invitation?.response ?? "new"}`} boy={boy} mentor={mentorName(data, boy.mentorId)} program={target} invitation={invitation} save={save} busy={busy} />; })}{visible.length === 0 && <Empty text={callFilter === "remaining" ? "No eligible boys remain to call for this session and mentor. Use Already called to review responses." : "No eligible boys match these filters."} />}</div>}
  </div>;
}
