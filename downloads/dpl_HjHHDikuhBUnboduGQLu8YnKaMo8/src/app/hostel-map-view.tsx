...
export default function HostelMapView({ boys, openProfile, ready }: { boys: MapBoy[]; openProfile: (id: string) => void; ready: boolean }) {
  ...
  return <div className="hostel-map-view">
    ...
    <p className="map-help">All registered boys keep their room marked, including dropped boys. Dropped registrations are labelled in the room and its details. Assign a floor to include an existing room registration.</p>
      ...
      <div className="map-scroll" tabIndex={0} role="region" aria-label={`${floor} room layout, scroll horizontally`}><div className="map-layout">
        {([{side:'OUT',groups:OUT_ROOM_GROUPS},{side:'IN',groups:IN_ROOM_GROUPS}]).map(({side,groups},index) => <div key={side}>
          ...
          <div className="map-side"><b>{side}</b><div className="map-room-groups">{groups.map((group,i) => <div className="map-room-group" key={i}>{group.map(number => {
            ...
            return <button key={number} className={`map-room room-${item.status}`} aria-label={`${floor} room ${number}: ${STATUS_LABELS[item.status]}, ${item.boys.length} of 2 registered, ${item.boys.filter(boy => boy.status === 'Dropped').length} dropped`} onClick={() => {setRoomKey(item.key);dialog.current?.showModal();}}><strong>{number}</strong><span>{item.boys.length}/2</span>{item.boys.some(boy => boy.status === 'Dropped') && <small className="map-dropped-label">{item.boys.filter(boy => boy.status === 'Dropped').length} dropped</small>}{item.status === 'data-issue' && <small>Issue</small>}</button>;
          })}</div>)}</div></div>
        </div>)}
      </div></div>
